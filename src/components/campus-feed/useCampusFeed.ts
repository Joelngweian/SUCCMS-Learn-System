...
const CAMPUS_POST_PAGE_SIZE = 10;
...
const signCampusPostMedia = async (
  posts: CampusPost[],
): Promise<CampusPost[]> => {
...
const signCampusCommentMedia = async (
...
const normalizeCampusPost = (row: CampusPostPageRow): CampusPost => ({
  id: row.id,
...
export function useCampusFeed() {
  ...
  const createPost = async () => {
    ...
    try {
      ...
      for (const media of selectedMedia) {
        const safeName = media.file.name.replace(/[^\w.-]+/g, "_");
      ...
      if (uploadedPaths.length > 0) {
        await supabase.storage.from("campus-posts").remove(uploadedPaths);
      }
      setComposerError(
        getNotifyMessage(
          error,
  ...
  const addComment = async (
    ...
    try {
      ...
      if (media) {
        const safeName = media.file.name.replace(/[^\w.-]+/g, "_");
        uploadedPath =
          `${userId}/comments/${postId}/${crypto.randomUUID()}_${safeName}`;
        const { error: uploadError } = await supabase.storage
          .from("campus-posts")
          .upload(uploadedPath, media.file);
        if (uploadError) throw uploadError;
        attachments.push({
          name: media.file.name,
          path: uploadedPath,
      ...
      if (uploadedPath) {
        await supabase.storage.from("campus-posts").remove([uploadedPath]);
      }
  ...
  const updateComment = async (
    ...
    try {
      ...
      if (newMedia) {
        const safeName = newMedia.file.name.replace(/[^\w.-]+/g, "_");
        uploadedPath =
          `${userId}/comments/${comment.postId}/${crypto.randomUUID()}_${safeName}`;
        const { error: uploadError } = await supabase.storage
          .from("campus-posts")
          .upload(uploadedPath, newMedia.file);
        if (uploadError) throw uploadError;
        attachments.push({
          name: newMedia.file.name,
          path: uploadedPath,
